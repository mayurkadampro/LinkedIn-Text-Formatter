window.addEventListener("load", () => {
    ...
    function toNormalText(text) {
        return text.replace(/[\uD835][\uDC00-\uDC9B]/g, match => {
            ...
            if (codePoint >= 0x1D400 && codePoint <= 0x1D419) { // Bold uppercase
                return String.fromCharCode('A'.charCodeAt(0) + (codePoint - 0x1D400));
            } else if (codePoint >= 0x1D41A && codePoint <= 0x1D433) { // Bold lowercase
                return String.fromCharCode('a'.charCodeAt(0) + (codePoint - 0x1D41A));
            } else if (codePoint >= 0x1D434 && codePoint <= 0x1D44D) { // Italic uppercase
                return String.fromCharCode('A'.charCodeAt(0) + (codePoint - 0x1D434));
            } else if (codePoint >= 0x1D44E && codePoint <= 0x1D467) { // Italic lowercase
                return String.fromCharCode('a'.charCodeAt(0) + (codePoint - 0x1D44E));
            } else if (codePoint >= 0x1D468 && codePoint <= 0x1D481) { // Bold Italic uppercase
                return String.fromCharCode('A'.charCodeAt(0) + (codePoint - 0x1D468));
            } else if (codePoint >= 0x1D482 && codePoint <= 0x1D49B) { // Bold Italic lowercase
    ...
    function formatText(type) {
        ...
        console.log("isMatchingStyle :- ", isMatchingStyle);


        const normalText = toNormalText(selectedText);
        const isFormatted = normalText !== selectedText;

        const resultText = isFormatted && isMatchingStyle ? normalText : applyFormatting(normalText, type);
